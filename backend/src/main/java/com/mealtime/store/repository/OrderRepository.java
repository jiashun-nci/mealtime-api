...
import com.mealtime.store.domain.Order;
...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(
        value = "select distinct jhiOrder from Order jhiOrder left join fetch jhiOrder.rider left join fetch jhiOrder.user",
        countQuery = "select count(distinct jhiOrder) from Order jhiOrder"
    )
    Page<Order> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct jhiOrder from Order jhiOrder left join fetch jhiOrder.rider left join fetch jhiOrder.user")
    List<Order> findAllWithToOneRelationships();

    @Query("select jhiOrder from Order jhiOrder left join fetch jhiOrder.rider left join fetch jhiOrder.user where jhiOrder.id =:id")
