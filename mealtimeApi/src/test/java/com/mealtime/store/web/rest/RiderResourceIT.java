...
class RiderResourceIT {
    ...
    @Mock
    private RiderRepository riderRepositoryMock;
    ...
    void getAllRidersWithEagerRelationshipsIsEnabled() throws Exception {
        when(riderServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRiderMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(riderServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllRidersWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(riderServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRiderMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(riderRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
