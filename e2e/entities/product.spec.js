...
describe('Product Screen Tests', () => {
  ...
  it('should allow you to create, update, and delete the Product entity', async () => {
    ...
    // active
    await scrollTo('activeInput', 'productEditScrollView');
    await toggleSwitchToValue('activeInput', true);
    // featured
    await scrollTo('featuredInput', 'productEditScrollView');
    await toggleSwitchToValue('featuredInput', false);
    // save
    ...
    // active
    await scrollTo('active', 'productDetailScrollView');
    await expect(element(by.id('active'))).toHaveLabel('true');
    // featured
    await scrollTo('featured', 'productDetailScrollView');
    await expect(element(by.id('featured'))).toHaveLabel('false');
    ...
    // price
    await scrollTo('priceInput', 'productEditScrollView');
    await element(by.id('priceInput')).replaceText('65537');
    await element(by.id('priceInput')).tapReturnKey();
    // productImage
    await scrollTo('productImageInput', 'productEditScrollView');
    // active
    ...
    // price
    await scrollTo('price', 'productDetailScrollView');
    await expect(element(by.id('price'))).toHaveLabel('65537');
    // productImage
    await scrollTo('productImage', 'productDetailScrollView');
    // active
    await scrollTo('active', 'productDetailScrollView');
