...
function ProductOrderDetailScreen(props) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.paddedScrollView} testID="productOrderDetailScrollView">
      ...
      <Text testID="user">{String(productOrder.user ? productOrder.user.name : '')}</Text>
      <Text style={styles.label}>Shopping Cart:</Text>
      <Text testID="shoppingCart">{String(productOrder.shoppingCart ? productOrder.shoppingCart.id : '')}</Text>
